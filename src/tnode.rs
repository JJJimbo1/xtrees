...
mod tnode {
    ...
    #[cfg(not(fearure = "hashbrown"))]
    use std::collections::HashMap;
    use crate::{BASE_CAPACITY, Dimension, Node};
    ...
    impl<I : Debug + Clone + Copy + Hash + PartialEq + Eq, D : Debug + Clone + Dimension> Node<I, D> for TNode<I, D> where [Self; D::SUB_COUNT]: Sized {
        ...
        fn subdivide(&mut self) {
            ...
            // self.nodes = Some(self.space.subdivisions().iter().map(|d| { Self::new(*d, self.capacity, self.depth_limit - 1) }).collect::<Vec<Self>>());
